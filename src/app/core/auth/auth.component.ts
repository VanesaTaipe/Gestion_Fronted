...
export default class AuthComponent implements OnInit {
  ...
  if (this.authType === 'completar') { //Nuevoooo Rodrigo

  const correoUsuario = this.userService.getCurrentUserEmail();

  if (!correoUsuario) {
    console.error("No se pudo obtener el correo del usuario actual."); //Nuevoooo Rodrigo
  ...
  if (this.authForm.value.password !== this.authForm.value.confirmPassword) { //Nuevoooo Rodrigo
  ...
  if (!/^\d{8}$/.test(this.authForm.value.dni || '')) {
    this.errors = { errors: { 'DNI': 'El DNI debe tener exactamente 8 dígitos.' } }; //Nuevoooo Rodrigo
    this.isSubmitting = false;
  return;
  }


  this.authForm.get('password')?.setValidators([ //Nuevoooo Rodrigo
        passwordStrengthValidator()
    ]);
    this.authForm.get('password')?.updateValueAndValidity();
  ...
  const payload = { //Nuevoooo Rodrigo
    user: {
  ...
  this.userService.updateTemporalUser(payload).subscribe({
    next: (res) => {
      ...
      this.isSubmitting = false;
      this.router.navigate(['/workspace']);     //Nuevoooo Rodrigo
    },
    error: (err) => {
  ...
  observable.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
    next: (res) => {
      ...
      const user: any = res?.user; //Nuevoooo Rodrigo 
